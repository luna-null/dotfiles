...
def format_chances(hour):
    ...
    probs = {text[e]: int(prob) for e, prob in hour.items()
             if e in chances and int(prob) > 0}
    sorted_probs = {e: probs[e]
...
for i, day in enumerate(weather['weather']):
    ...
    for hour in day['hourly']:
        ...
        data['tooltip'] += f"{format_time(hour['time'])} {WEATHER_CODES[hour['weatherCode']]} {
            format_temp(hour['FeelsLikeC'])} {hour[weather_desc][0]['value']}, {format_chances(hour)}\n"
